...
def main():
    ...
    calculating_set = all_pos_at_checkpoints[:, checkpoint]
    plt.scatter(list(range(trials)), calculating_set)
    plt.xlabel("trial id")
    plt.ylabel("x position (@ t = {:d})".format(checkpoints[checkpoint]))
    plt.title("x position vs. trials as of {:d} bits".format(bits))
    plt.show()
