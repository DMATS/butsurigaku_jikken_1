...
def random_walk(local_rands, bits, checkpoints):
    x=0
    pos_at_checkpoints = [0]
    for t in range(1, max(checkpoints)+1):
        if local_rands[(t-1)%(2**bits)] & 1 == 0: # this is the equivalent of "local_rands[] % 2 == 1"
            x = x+1
        else:
            x = x-1
...
def main():
    ...
    trials = 5
    # checkpoints = cycle
    checkpoints = [0,30,50,int(0.3*cycle),int(0.5*cycle),int(0.7*cycle),cycle]
    checkpoints.sort()
    markers = ['+','x','D','d']
    ...
    for i in range(trials):
        ...
        pos_at_checkpoints = random_walk(local_rands, bits, checkpoints)
        # plt.plot(checkpoints, pos_at_checkpoints, label = "inital number = {:d}".format(init))
        plt.scatter(checkpoints, pos_at_checkpoints, alpha=0.5, marker = markers[i%len(markers)], label = "inital number = {:d}".format(init))
    plt.xlabel("time")
